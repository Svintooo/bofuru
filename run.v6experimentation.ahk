...
if true
{
  TextBackgroundColor := 0x000000  ; A custom color in BGR format.
  TextBackgroundBrush := DllCall("CreateSolidBrush", "UInt", TextBackgroundColor)

  MyGui := Gui()
  MyGui.BackColor := "Black"
  ;Text := MyGui.Add("Text",, "Here is some text that is given`na custom background color.")
  Text := MyGui.Add("Button", "Default", "Here is a button that is given`na custom background color.")
  ...
  WindowProc(hwnd, uMsg, wParam, lParam)
  {
    ...
    if (lParam = Text.Hwnd && (306 <= uMsg && uMsg <= 312))  ; WM_CTLCOLOR(MSGBOX|EDIT|LISTBOX|BTN|DLG|SCROLLBAR|STATIC).
    {
      ;DllCall("gdi32\SetTextColor", "Ptr", wParam, "UInt", TextColor)
