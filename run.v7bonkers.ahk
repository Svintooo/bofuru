...
if true
{
  RGB_to_ABGR(int)
  {
    ...
    Return (
        (int & 0xff0000) >> 16 |  ; RGB => ..R
        (int & 0x00ff00)       |  ; RGB => .G.
        (int & 0x0000ff) << 16 |  ; RGB => B..
        0xff000000                ;       A...
      )
  }
  ...
  ;win_bgbrush := DllCall("CreateSolidBrush", "UInt", RGB_to_ABGR(win.BackColor))

  ;button := win.Add("Text",, "Here is some text that is given`na custom background color.")
  button := win.Add("Button", "Default", "Here is a button that is given`na custom background color.")
  ...
  ;WindowProcOld := DllCall(SetWindowLong, "Ptr", win.Hwnd, "Int", -4  ; -4 is GWL_WNDPROC
  ...
  ;  
  ;  if IsSet(button)  ; Prevent error on exit: "This variable has not been assigned a value."
  ;  && lParam = button.Hwnd
  ...
  ;    DllCall("SetBkColor", "Ptr", wParam, "UInt", RGB_to_ABGR(win.BackColor))
...
if false
{
  ...
  ;  bg_brush := DllCall("CreateSolidBrush", "UInt", bg_color)
  ;  SetWindowLong := A_PtrSize=8 ? "SetWindowLongPtr" : "SetWindowLong"
  ...
  ;  {
  ;    Critical
  ;    if (uMsg = 0x0138 && lParam = btnX.Hwnd)  ; 0x0138 is WM_CTLCOLORSTATIC.
  ;    {
  ;      MsgBox("Alive?")
  ;      DllCall("SetBkColor", "Ptr", wParam, "UInt", bg_color)
  ;      return bg_brush  ; Return the HBRUSH to notify the OS that we altered the HDC.
  ;    }
  ;    ; Otherwise (since above didn't return), pass all unhandled events to the original WindowProc.
  ;    return DllCall("CallWindowProc", "Ptr", WindowProcOld, "Ptr", hwnd, "UInt", uMsg, "Ptr", wParam, "Ptr", lParam)
  ;  }
